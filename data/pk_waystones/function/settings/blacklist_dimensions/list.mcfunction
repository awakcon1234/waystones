...
execute unless score $length pk.temp matches 1.. run tellraw @s {"text":"Hiện chưa có chiều không gian nào trong danh sách cấm","color":"gray"}
...
execute if score $length pk.temp matches 1.. run tellraw @s [{"score":{"name":"$length","objective":"pk.temp"},"color":"yellow"},{"text":" chiều không gian bị cấm:\n","color":"gray"},{"nbt":"temp.array_1","storage":"pk:common","color":"gray"}]